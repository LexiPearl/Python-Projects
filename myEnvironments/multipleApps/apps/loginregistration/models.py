...
class RegistrationManager(models.Manager):
    def register(self, first_name, last_name, email, password, confirm_password):
        message = []
        print type(password), "---password---",type(password.encode()), type(str(password))
        ...
        if len(message) != 0:
        	...
        	registration = User.registerMgr.create(first_name=first_name,last_name=last_name, email=email, password=pw_hash)
        	registration.save()
...
class LoginManager(models.Manager):
    def login(self, email, password):
        ...
        if  len(email) <1 and len(password) < 1:
            login_message.append("Email and Password cannot be blank!")
        if len(email) == 0:
            login_message.append("Email is required")
            if len(password) == 0:
        ...
        if len(login_message) != 0:
            ...
            print "False======"
            login = User.loginMgr.filter(email=email)
            if len(login) < 1:
